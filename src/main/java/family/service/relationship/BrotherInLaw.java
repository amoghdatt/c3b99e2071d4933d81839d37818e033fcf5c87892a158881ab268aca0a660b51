...
public class BrotherInLaw extends Relationship {
    ...
    public ArrayList<String> getPeople(FamilyMember member) {
        ...
        if(siblings != null) {
            combinedArray = getAllSpousesOfMembersBasedOnGender(siblings, "Male");
        }
        if(siblingsOfSpouse != null){
            combinedArray.addAll(getAllMembersFromArrayBasedOnNameAndGender(siblingsOfSpouse, spouse.getName(), "Male"));
        }
